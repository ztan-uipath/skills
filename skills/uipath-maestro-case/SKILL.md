...
1. **Phase 0 interview when `sdd.md` absent.** Generate `sdd.md` via a guided interview (orient → listen and ground → three confirmation packets → reconcile → approve). Confirm the case in plain language before rendering the full draft; run Finalization against `sdd.draft.md`; create `sdd.md` only after explicit approval. Never overwrite an existing `sdd.md`.
...
3. **PHASE 1 HARD GATE — refresh registry before planning.** Run `uip login status --output json`, then `uip maestro case registry pull`, before cache inspection, carryover reuse, resource resolution, or any Phase 1 artifact write. Trust the SDD as written; the pull refreshes the local discovery cache and does not validate or override the SDD. **Cache-state rule:** before a successful pull, a missing cache directory/file is a failed refresh precondition — never a zero-match result. Only after a successful pull may an empty exact-name match set (or a still-absent type index) enter the normal empty-lookup flow. Login/pull failure → surface it and stop Phase 1. Discovery reads `~/.uip/case-resources/<type>-index.json` directly because `registry search` has known gaps (esp. action-apps). Phase 0 begins the same login/pull and connection grounding during Listen, then fans out independent candidate and schema discovery before the confirmation that depends on it; this improves the interview but never replaces the Phase 1 hard gate. See [references/registry-discovery.md](references/registry-discovery.md).
...
14. **Validate `bindings_v2.json` before every `uip solution resources refresh`.** Run Step 12 Check 7; on mismatch, regenerate once and re-check, then halt if it still fails. Always run `resources refresh` before `uip solution upload` or `uip maestro case debug` so Studio Web can resolve dependencies.
...
|---|---|
| New case without an SDD | `1. Understand the case and map available UiPath resources. 2. Confirm the design in three short summaries. 3. Generate and validate the SDD. 4. Build and review the case. 5. Ask before running or publishing anything.` |
| New case with an SDD | `1. Read the design and verify available UiPath resources. 2. Build the implementation plan. 3. Build and validate the case. 4. Ask before running or publishing anything.` |
...
Triggered when `sdd.md` absent at resolved path. Read [references/phase-0-interview.md](references/phase-0-interview.md) for the interview modes (orient → listen and ground → three confirmation packets → reconcile → approve), resumption, and HTML preview offer. Produces:

> **Read budget for Phase 0.** Read `phase-0-interview.md`, `references/sdd-generation-rules.md` (the mental model + task-type reasoning the confirmation packets rely on), and `assets/templates/sdd-template.md` to begin the interview. Read these independent references in parallel. Do NOT preload plugin `impl-json.md` files — those are needed only in Phase 2/3 and pulled in just-in-time per T-entry.

> **Early tenant grounding.** Open the interview immediately. After the user's first description, follow [phase-0-interview.md § Early tenant grounding](references/phase-0-interview.md#early-tenant-grounding-and-parallel-discovery): overlap document reads with login status, pull only after login succeeds, then fan out independent resource/connection searches and selected-resource schema discovery. Resolve before the confirmation that depends on the tenant result. Phase 1 still refreshes and verifies everything before planning.

- `sdd.md` — generated against `assets/templates/sdd-template.md`
- `tasks/registry-resolved.json` — per-task registry resolutions
- `sdd.draft.md` — intermediate, atomically promoted to `sdd.md` only after approval
