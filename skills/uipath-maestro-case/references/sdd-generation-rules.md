...
- **Exits via `return-to-origin`** — routes the case back to the origin stage, through the exit rule, not a new edge. Since secondary stages are interrupting lanes, return lanes already satisfy the required `Interrupting: Yes`.
