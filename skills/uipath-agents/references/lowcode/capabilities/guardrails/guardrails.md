...
14. **Always run `uip solution resources get "<Key>" --output json | python3 /tmp/verify_escalation_app.py` (Step 2) before deciding on any escalation guardrail** — this is mandatory regardless of whether you intend to write or reject the guardrail. `uip solution resources list` output alone is never sufficient to determine schema compatibility. Run `resources get` + the verifier, then act on the exit code: 0 = proceed, 1 = report `<APP_NAME> does not have the required action schema configuration for tool guardrails.` and stop. See [§ Adding an escalation guardrail — Step 2](#adding-an-escalation-guardrail--step-by-step).
