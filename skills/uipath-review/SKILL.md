...
| Coded (`main.py` + framework config) | `timeout 300 uip codedagent review "<PROJECT_DIR>" --output json` |
...
> **If the CLI exits non-zero or is killed by the `timeout` wrapper** (exit 124 or any non-zero exit), proceed immediately to Step 2.5b — read source files directly and apply the judgment catalog without CLI output. Do **NOT** retry the CLI. Record "Review CLI unavailable: [exit code/timeout]" under Rules Skipped (Critical Rule 11). The judgment catalog and direct source-file analysis cover most review findings independently of CLI output.
