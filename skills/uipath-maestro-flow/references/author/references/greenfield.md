...
|---|---|---|
| **T1 — Setup + discovery** | 0, 1, 2, 3 | **Precondition: run the Step 2 solution-discovery gate FIRST — `find . -maxdepth 2 -type f -name '*.uipx' -print`. If it finds any solution, the gate (ask, await the answer) is a required turn before this chain — do NOT scaffold until answered.** Then: one chained `Bash` (scaffold + register + pull + `node add` for each CLI-owned node) **+** parallel `Bash` (one `registry get` per OOTB type you'll inline) **+** parallel `Read` (plugin `impl.md`s) **+** optional `uip login status`. |
...
> **A Flow project cannot exist outside a solution** (universal rule in [SKILL.md](../../../SKILL.md)). Run `uip maestro flow init` (Step 2b) outside a solution and it now **auto-scaffolds** one — `<Project>Solution/<Project>Solution.uipx` with the project nested at `<Project>Solution/<Project>/` (response carries `Data.AutoCreatedSolution`). Still scaffold or select the solution first (Step 2a) so the solution name matches the project name rather than the auto `<Project>Solution`, and so discovery is unambiguous. The correct layout is **always** `<Solution>/<Project>/<Project>.flow` (double-nested — see the tree after Step 2c). Passing `--skip-solution-registration` opts out of both auto-scaffold and registration, leaving a bare single-nested layout that fails Studio Web upload and packaging.

Check for existing solutions with `find . -maxdepth 2 -type f -name '*.uipx' -print` (each solution is its own folder — `<Solution>/<Solution>.uipx` — so a bare `ls *.uipx` misses them, and in zsh an unmatched top-level glob aborts the whole command, silently returning nothing). If any are found, **STOP before the T1 chain — do not run `uip solution init` yet** — and ask the user (dropdown per SKILL.md rule #5), presenting one option per discovered `.uipx`, a **"Create a new solution"** option, and **"Something else"** as the last option. Use the runtime's structured-question tool (`AskUserQuestion` on Claude) when available; otherwise ask the same numbered choices in chat. The user wanting a new solution does not let you skip this; you only learn that by asking. If none are found, create a new one automatically. See the dropdown question rule in [SKILL.md](../../../SKILL.md).
